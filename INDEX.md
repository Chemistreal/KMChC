...
> ⚠ **이 글의 경로는 지금 저장소가 아니다**(2026-08-10 확인). "모든 파일은
> `/mnt/user-data/outputs/`" 는 산출물을 zip 으로 내보내던 시절의 자리다.
> 무엇을 만들었는지의 차례로는 쓸 만하지만, **어디에 있는지는 여기서 찾지
> 않는다** — 지금 모습은 `README.md` 에 있다.
