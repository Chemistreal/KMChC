...
KNOWN = {
    # 이 앱은 검사를 보고 그 자리에서 Apps Script 로 보낸다. 리포트는 나중에
    # 짧은 주소로 열고, 그때 읽는 것은 저장소의 report-data/ 다. 그러니 이
    # 브라우저에 남는 것은 **보다 만 검사**뿐이다 — 남의 기록은 안 남는다.
    'kmchc_v2_progress': '검사를 어디까지 봤나. 창을 닫아도 이어서 볼 수 있게 둔다',
    'kmchc_v2_last':     '마지막으로 본 문항. 위와 같다',
    'kmchc_v2_log':      '문항마다 머문 시간·고친 횟수. 응답 타당도를 재는 데 쓴다',
}
