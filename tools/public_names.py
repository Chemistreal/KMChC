...
2026-08-11 에 네 저장소를 한꺼번에 재어 보니, 실명이 나온 곳은 여기 하나였다.
  ...
  ② report-data/*.json  쉰여덟 장 가운데 이름이 실린 것 쉰여섯, 사람 쉰셋.
     (나머지 둘은 이름 칸이 비었거나 `test` 다. 쉰셋은 `김지완` 과
      `김지완(내정중)` 을 한 사람으로 센 값이다 — 같은 반 동명이인이라
      선생님이 손으로 갈라 적어 두셨다.)
...
#   `"name":"김지완(내정중)"` 한 장이 통째로 안 보였다. 같은 반에 김지완이
#   둘이라 손으로 갈라 적어 둔 것인데, 갈라 적을 만큼 신경 쓴 이름일수록
...
#   걸렀다. 그랬더니 **김예성 · 이지율 · 임하율 · 황윤성** 넷이 용어로 걸러져
