/* ============================================================
   ...
   ------------------------------------------------------------
   `tools/theme.py` 가 이 저장소의 화면에 그림을 깔았다.
...
(async () => {
  const srv = spawn(process.execPath, ['-e', `
    ...
    const T={'.html':'text/html; charset=utf-8','.js':'text/javascript','.json':'application/json','.css':'text/css','.png':'image/png','.svg':'image/svg+xml'};
  ...
  try {
    for (const [file, label] of PAGES) {
      ...
      /* 검사가 **진짜 시트에 쓰면 안 된다.** exam 저장소의 _seal.js 와 같은 뜻으로
         구글로 나가는 길을 여기서 끊는다. */
      ...
      if (!dark) chk('먹색이 한 벌이다', on.ink === '#1c1812', on.ink);
